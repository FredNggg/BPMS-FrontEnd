...
export const getAllMarketers = function(currPage, pageSize = 10){
	return request({
		url: `${PREFIX}/marketer/all?currPage=${currPage}&pageSize=${pageSize}`,
		method: 'GET',
	})
}
...
export const getMarketersByState = function(state, currPage, pageSize = 10){
	return request({
		url: `${PREFIX}/marketer/${state}?currPage=${currPage}&pageSize=${pageSize}`,
		method: 'GET',
	})
}
...
export const modifyState = function(id, state){
	return request({
		url: `${PREFIX}/marketer/state/${id}?state=${state}`,
		method: 'POST',
	})
}