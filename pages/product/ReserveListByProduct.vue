...
<template>
	<view>
		<view style="margin-left: 40rpx;">该产品的预约记录：</view>
		<view>
			<u-collapse>
				<u-collapse-item v-for="(item, index) in records" :title="item.customerName" :value="item.reserveTime">
					<u--form label-width="180rpx">
						...
						<u-form-item label="银行卡号">{{item.cardId}}</u-form-item>
						<u-form-item label="身份证号">{{item.customerId}}</u-form-item>
...
<script>
	...
	export default {
		...
		onLoad(option) {
			...
			getProductReserveList(this.productId, this.currPage).then(res => {
				this.records = res.data.records;
			})
		},
		onReachBottom() {
			getProductReserveList(this.productId, this.currPage + 1).then(res => {
